...
namespace Xt30Probe.Presentation
{
    public sealed class MainForm : Form
    {
        ...
        public MainForm(bool offline,string dataDirectory)
        {
            ...
            Library=new RecipeLibrary(dataDirectory??Path.Combine(AppDomain.CurrentDomain.BaseDirectory,"data"));
            ...
            _cameraPage.Controls.AddRange(new Control[]{_overview,_recent,_quick,_custom});_slotsPage.Controls.Add(_slots);
            ...
            BuildBackups();BuildSettings();Camera.Changed+=delegate{UpdateCamera();};Library.Changed+=delegate{_recent.Grid.Invalidate(true);_custom.Invalidate(true);_slots.Invalidate(true);};
            ...
            Shown+=delegate{Camera.Start();if(Library.LoadWarning!="")MessageBox.Show(this,Library.LoadWarning,"Library recovery");};
        ...
        protected override void OnLayout(LayoutEventArgs e)
        {
            ...
            _readOnly.SetBounds(_header.Width-230,32,117,34);_help.SetBounds(_header.Width-93,32,35,34);_menu.SetBounds(_header.Width-48,32,30,34);LayoutCamera();LayoutSlots();
        }
        void PaintHeader(object sender,PaintEventArgs e)
        {Theme.TextAt(e.Graphics,CurrentPage=="Camera"?(Camera==null?"FUJIFILM X-T30":Camera.State.Name):CurrentPage,29,true,Theme.Text,new Rectangle(34,24,_header.Width-280,43));}
        void LayoutCamera()
        {
        ...
        void LayoutSlots(){if(_slots==null)return;_slots.SetBounds(28,_slotsPage.AutoScrollPosition.Y,Math.Min(660,_slotsPage.ClientSize.Width-56),822);_slotsPage.AutoScrollMinSize=new Size(0,846);}
        void UpdateCamera()
        {
        ...
        void BuildBackups()
        {
            ...
            card.Paint+=delegate(object s,PaintEventArgs e){Theme.TextAt(e.Graphics,"Local library backups",19,true,Theme.Text,new Rectangle(24,20,600,30));Theme.Lines(e.Graphics,"Save a snapshot of your local recipes and favorites. This does not read or restore camera settings.",14,Theme.Muted,new Rectangle(24,60,card.Width-48,50));};
        ...
        void RefreshBackups(){_backupList.Items.Clear();string dir=Path.Combine(Library.DirectoryPath,"backups");if(Directory.Exists(dir))foreach(string file in Directory.GetFiles(dir,"*.json").OrderByDescending(x=>x))_backupList.Items.Add(Path.GetFileName(file));if(_backupList.Items.Count==0)_backupList.Items.Add("No local backups yet.");}
        void BuildSettings()
        {
            RoundedCard card=new RoundedCard(){Location=new Point(28,0),Size=new Size(780,344),Anchor=AnchorStyles.Top|AnchorStyles.Left|AnchorStyles.Right};_settings.Controls.Add(card);
            card.Paint+=delegate(object s,PaintEventArgs e){Theme.TextAt(e.Graphics,"Application settings",19,true,Theme.Text,new Rectangle(25,20,500,35));Theme.TextAt(e.Graphics,"Appearance",14,true,Theme.Text,new Rectangle(25,83,220,26));Theme.TextAt(e.Graphics,"Light · Segoe UI · Fujifilm green",14,false,Theme.Muted,new Rectangle(255,83,card.Width-280,26));Theme.TextAt(e.Graphics,"Camera access",14,true,Theme.Text,new Rectangle(25,132,220,26));Theme.TextAt(e.Graphics,"Read only — always enforced by the engine",13,false,Theme.Muted,new Rectangle(255,132,card.Width-280,26));Theme.Lines(e.Graphics,"Extended scanning uses the existing property sweep. It takes longer and does not enable any camera writes.",13,Theme.Muted,new Rectangle(25,243,card.Width-50,58));};
            CheckBox sweep=new CheckBox(){Text="Extended property scan (existing --sweep)",Checked=Library.ExtendedScan,Location=new Point(26,196),Size=new Size(560,29),Font=Theme.Font(14,false)};card.Controls.Add(sweep);sweep.CheckedChanged+=delegate{try{Library.ExtendedScan=sweep.Checked;Library.Save();}catch(Exception ex){MessageBox.Show(this,ex.Message,"Settings could not be saved");}};
        }
